...
Occurrence_isNone=set(record["occurrenceID"] for record in clean_records if record["occurrenceID"] is None )
print(f"None ocurrence id: {len(Occurrence_isNone)}")
Occurrence_isNotNone=set(record["occurrenceID"] for record in clean_records if record["occurrenceID"] is not None )
print(f"None ocurrence id: {len(Occurrence_isNotNone)}")

Occurrence_hasID_list = [record["occurrenceID"] for record in clean_records if record["occurrenceID"] is not None]

if len(Occurrence_hasID_list ) != len(Occurrence_isNotNone):
    duplicate_count = len(Occurrence_hasID_list) - len(Occurrence_isNotNone)
    print(f"There are {len(duplicate_count)} ID duplicates")
